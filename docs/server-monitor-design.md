...
- Manual inventory refresh and parallel queue reconciliation run as background work. The UI polls their short status endpoints instead of holding one reverse-proxy request open for all SSH checks and dispatch attempts.
